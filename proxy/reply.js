...
exports.getRepliesByNewsId = function (id, cb) {
  Reply.find({news_id: id, deleted: false}, '', {sort: 'create_at'}, function (err, replies) {
    if (err) {
      return cb(err);
    }
    if (replies.length === 0) {
...
exports.getRepliesByCUITId = function (id, cb) {
  Reply.find({cuit_id: id, deleted: false}, '', {sort: 'create_at'}, function (err, replies) {
    if (err) {
      return cb(err);
    }
    if (replies.length === 0) {
...
exports.getRepliesByExampleId = function (id, cb) {
  Reply.find({example_id: id, deleted: false}, '', {sort: 'create_at'}, function (err, replies) {
    if (err) {
      return cb(err);
    }
    if (replies.length === 0) {
