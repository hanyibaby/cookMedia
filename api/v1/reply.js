...
var create = function (req, res, next) {
  ...
  var cuit_id = req.params.cuit_id;
  var example_id = req.params.example_id;
  ...
  if (!validator.isMongoId(topic_id||news_id||example_id||cuit_id)) {
    res.status(400);
  ...
  News.getNews(news_id, ep.done(function (news) {
    if (!news) {
      res.status(404);
      return res.send({success: false, error_msg: '新闻不存在'});
    }
    if (news.lock) {
      res.status(403);
      return res.send({success: false, error_msg: '该新闻已被锁定'});
    }
    ep.emit('news', news);
  }));

  CUIT.getCUIT(cuit_id, ep.done(function (cuit) {
    if (!cuit) {
      res.status(404);
  ...
  Example.getExample(example_id, ep.done(function (example) {
    if (!example) {
      res.status(404);
      return res.send({success: false, error_msg: '资源不存在'});
    }
    if (example.lock) {
      res.status(403);
      return res.send({success: false, error_msg: '该资源已被锁定'});
    }
