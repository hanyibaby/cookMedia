...
var CUITModel   = require('../models').CUIT;
...
exports.index = function (req, res, next) {
  ...
  User.getUserByLoginName(user_name, function (err, user) {
    ...
    var render = function (recent_topics, recent_replies) {
      ...
      if(user.is_admin){
        res.render('user/index', {
          user: user,
          recent_newss: recent_topics,
          recent_cuits: recent_topics,
          recent_examples: recent_topics,
          token: token,
   ...
   //////////////////hanyi1/18
    if(user.is_admin){
     // proxy.assign('recent_newss',render);
      proxy.assign('recent_examples',render);
    //  proxy.assign('recent_cuits',render);
    }
    else{
  ...
  ///////////////////hanyi1/18
    ...
    News.getNewssByQuery(query, opt, proxy.done('recent_newss'));
    CUIT.getCUITsByQuery(query, opt, proxy.done('recent_cuits'));
    Example.getExamplesByQuery(query, opt, proxy.done('recent_examples'));
...
exports.listCollectedNewss = function (req, res, next) {
  var name = req.params.name;
  var page = Number(req.query.page) || 1;
  ...
  User.getUserByLoginName(name, function (err, user) {
    ...
    var pages = Math.ceil(user.collect_news_count/limit);
    var render = function (newss) {
      res.render('user/collect_newss', {
        newss: newss,
        current_page: page,
        pages: pages,
        user: user
    ...
    var proxy = EventProxy.create('newss', render);
    proxy.fail(next);
    ...
    NewsCollect.getNewsCollectsByUserId(user._id, opt, proxy.done(function (docs) {
      var ids = docs.map(function (doc) {
        return String(doc.news_id)
      })
      var query = { _id: { '$in': ids } };

      News.getNewssByQuery(query, {}, proxy.done('newss', function (newss) {
...
exports.listCollectedCUITs = function (req, res, next) {
  var name = req.params.name;
  var page = Number(req.query.page) || 1;
  ...
  User.getUserByLoginName(name, function (err, user) {
    ...
    var pages = Math.ceil(user.collect_cuit_count/limit);
    var render = function (cuits) {
      res.render('user/collect_cuits', {
        cuits: cuits,
        current_page: page,
        pages: pages,
        user: user
    ...
    var proxy = EventProxy.create('cuits', render);
    proxy.fail(next);
    ...
    CUITCollect.getCUITCollectsByUserId(user._id, opt, proxy.done(function (docs) {
      var ids = docs.map(function (doc) {
        return String(doc.cuit_id)
      })
      var query = { _id: { '$in': ids } };

      CUIT.getCUITsByQuery(query, {}, proxy.done('cuits', function (cuits) {
...
exports.listCollectedExamples = function (req, res, next) {
  var name = req.params.name;
  var page = Number(req.query.page) || 1;
  ...
  User.getUserByLoginName(name, function (err, user) {
    ...
    var pages = Math.ceil(user.collect_example_count/limit);
    var render = function (examples) {
      res.render('user/collect_examples', {
        examples: examples,
        current_page: page,
        pages: pages,
        user: user
    ...
    var proxy = EventProxy.create('examples', render);
    proxy.fail(next);
    ...
    ExampleCollect.getExampleCollectsByUserId(user._id, opt, proxy.done(function (docs) {
      var ids = docs.map(function (doc) {
        return String(doc.example_id)
      })
      var query = { _id: { '$in': ids } };

      Example.getExamplesByQuery(query, {}, proxy.done('examples', function (examples) {
...
exports.listNewss = function (req, res, next) {
  var user_name = req.params.name;
  var page = Number(req.query.page) || 1;
  var limit = config.list_news_count;
  ...
  User.getUserByLoginName(user_name, function (err, user) {
    ...
    var render = function (newss, pages) {
      res.render('user/newss', {
        user: user,
        newss: newss,
        current_page: page,
        pages: pages
    ...
    var proxy = new EventProxy();
    proxy.assign('newss', 'pages', render);
    proxy.fail(next);

    var query = {'author_id': user._id};
    var opt = {skip: (page - 1) * limit, limit: limit, sort: '-create_at'};
    News.getNewssByQuery(query, opt, proxy.done('newss'));
...
exports.listCUITs = function (req, res, next) {
  var user_name = req.params.name;
  var page = Number(req.query.page) || 1;
  var limit = config.list_cuit_count;
  ...
  User.getUserByLoginName(user_name, function (err, user) {
    ...
    var render = function (cuits, pages) {
      res.render('user/cuits', {
        user: user,
        cuits: cuits,
        current_page: page,
        pages: pages
    ...
    var proxy = new EventProxy();
    proxy.assign('cuits', 'pages', render);
    proxy.fail(next);

    var query = {'author_id': user._id};
    var opt = {skip: (page - 1) * limit, limit: limit, sort: '-create_at'};
    CUIT.getCUITsByQuery(query, opt, proxy.done('cuits'));
...
exports.listExamples = function (req, res, next) {
  var user_name = req.params.name;
  var page = Number(req.query.page) || 1;
  var limit = config.list_example_count;
  ...
  User.getUserByLoginName(user_name, function (err, user) {
    ...
    var render = function (examples, pages) {
      res.render('user/examples', {
        user: user,
        examples: examples,
        current_page: page,
        pages: pages
    ...
    var proxy = new EventProxy();
    proxy.assign('examples', 'pages', render);
    proxy.fail(next);

    var query = {'author_id': user._id};
    var opt = {skip: (page - 1) * limit, limit: limit, sort: '-create_at'};
    Example.getExamplesByQuery(query, opt, proxy.done('examples'));
