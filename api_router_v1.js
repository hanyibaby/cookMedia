...
var newsCollectController   = require('./api/v1/news_collect');
...
// 新闻
router.get('/newss', newsController.index);
router.get('/news/:id', middleware.tryAuth, newsController.show);
router.post('/newss', middleware.auth, limit.peruserperday('create_news', config.create_post_per_day, {showJson: true}), newsController.create);
router.post('/newss/update', middleware.auth, newsController.update);


// 新闻收藏
router.post('/news_collect/collect', middleware.auth, newsCollectController.collect); // 关注某话题
router.post('/news_collect/de_collect', middleware.auth, newsCollectController.de_collect); // 取消关注某话题
router.get('/news_collect/:loginname', newsCollectController.list);
...
router.post('/news/:news_id/replies', middleware.auth, limit.peruserperday('create_reply', config.create_reply_per_day, {showJson: true}), replyController.create);
